...
data(pavela)
# Note that `pavela` is a data element contained within the `bulkUploader` package

neotoma <- pavela$PUBLIC
...
# for pavela, removed citation and doi
...
# Blocks of code that didn't work!!
pubList <- formatPubs(neotoma, 
                      year = 'AÑO_PUBLICACION',
                      title = c('TITULO', 'NOMBRE_LIBRO.REVISTA')) # note: this is not working with two elements. The coalesce function isn't working for me.


outputs <- scrape_dois(pubList[1:10,],
            n = 3, 
            savefile = 'data/pavelatab.RDS',   # argh, this isn't working. Could it be because of the encoding? See this note in readRDS: "If file is a file name, it is opened by gzfile except for save(compress = FALSE) which uses file. Only for the exception are marked encodings of file which cannot be translated to the native encoding handled on Windows."
            restore = FALSE)
...
neoCites <- makeCite(pubList$citation)  # but I don't have a citation column in my data. use query.bibliographic instead?
...
# Notes: the prompt is to accept the *best match*. So why are we also showing the "next match"? This is blurring the matching process for me. When I say "n", it doesn't then ask me about the next match.
```
