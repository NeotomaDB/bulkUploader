...
# reconcile columns
author <- "Author" # column name that corresponds to first author name
year <- "Year" # column name that corresponds to year
title  <- "Title" # column name that corresponds to article title
journal <- "Journal" # column name that corresponds to journal
booktitle <- "Book.Title" # column name that corresponds to book title (if different than the article title column)

dat <- originalcsv %>%
  select(author = as.character(author), year = as.character(year), title = as.character(title), journal = as.character(journal))

#I think this could work - turn this into a "convertdata" function??
```
...
results <- data.frame(score=rep(NA, nrow(originalcsv)), delta = NA)
outputs <- list()
...
for (i in (length(outputs) + 1):nrow(originalcsv)) {
  if (is.na(results$score[i])) {
    # Just testing to see if there's a title.
    container = ifelse(is.na(originalcsv[i, which(colnames(originalcsv)==journal)]),
                       originalcsv[i, which(colnames(originalcsv)==booktitle)],
                       originalcsv[i, which(colnames(originalcsv)==journal)])
    
    if (is.na(container)) {
      message("Row ", i, " is missing a title.")
    } else {
      outputs[[i]] <- scrape_dois(title = originalcsv[i, which(colnames(originalcsv)==title)],
                          year = originalcsv[i, which(colnames(originalcsv)==year)])
...
## This code is not working, I think becaase tibbles handle retrieving data by column name vs number differently than a dataframe?
## Maybe I need to use "select" function here instead? that's probably a better option anyways.
# miocites <- makeCite(originalcsv[,which(colnames(originalcsv)==author)], 
#                      originalcsv[,which(colnames(originalcsv)==year)], 
#                      originalcsv[,which(colnames(originalcsv)==title)], 
#                      originalcsv[,which(colnames(originalcsv)==journal)])
