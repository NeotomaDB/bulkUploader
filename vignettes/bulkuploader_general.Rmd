...
convertdata <- function(x, author, year, title, journal){ # no booktitle yet!
  x <- x %>%
...
testdat <- convertdata(miomapcsv, "Author", "Year", "Title", "Journal")
```
...
for (i in (length(outputs) + 1):10){ #nrow(originalcsv)) {
  if (is.na(results$score[i])) {
    # Just testing to see if there's a title.
    container = ifelse(is.na(originalcsv[i, 'journal')]),
