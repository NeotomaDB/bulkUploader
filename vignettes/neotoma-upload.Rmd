...
#I do not think this line of code is working correctly It just makes MatchedPubs into only line 2 and does not keep the other 9 citations.
...
## Putting in contacts
Two contact categories:
Publication related contacts such as Authors and Editors, and Contacts for the dataset.

First we will use the matched publication data to pull the author information and match that to neotoma.

Pulling the authors from the publications. The contacts need to be formatted as?
```{r authorcontacts}

#there is a mismatch between the paths so I had to change x@author to x@author@authors
pullAuth <- function(x) {
  x@author@authors %>%
...
pullAuth <- function(x) {
  x@author@authors %>%
    map(function(y) paste(y@author@familyname))
}

contact <- pullAuth(matchedPubs)


```

Now we can match them to neotoma
```{r getNeotomaMatches}

#I am not sure what the data structure needs to be
matchedContacts <- get_contacts(contact[[1]])


```

user has to select matches?
```{r assignMatches}
showMatch(matchedContacts[[2]])

#I do not think this line of code is working correctly It just makes MatchedPubs into only line 2 and does not keep the other 9 citations.
matchedContacts[[2]] <- selectMatch(matchedContacts[[2]], 1)
showMatch(matchedContacts[[2]])
```

many contacts will probably not match up with neotoma right away because of slight differences in name? so this is a first pass and then there will be a lot of contacts that didn't match.
